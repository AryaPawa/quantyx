...
class StochasticVarianceModel : # go through and undertsand again #
    ...
    def __init__(self, S, mu, r, div, alpha, beta, rho, vol_var, inst_var, dt, T):
        self.simulated_path= self.simulate_path(S, mu, r, div, alpha, beta, rho, vol_var,dt, T)
...
class MonteCarloPut :
    ...
    def simualte_price_gbm(self, strike, n, r , S, mu, sigma, dt, T) :
        payouts = []
        for i in range(0 , n) :
            GBM = GeometricBrownianMotion(S, mu, sigma, dt , T)
            if(GBM.simulated_path[-1] <= strike) :
    ...
    def simulate_price_svm(self, strike, n , S, mu, r , div, alpha, beta, rho, vol_var, inst_var, dt, T) :
            ...
            for i in range(0, n) :
                SVM = StochasticVarianceModel(S, mu, r, div, alpha, beta, rho, vol_var, inst_var, dt, T)
                if(SVM.stimulated_path[-1] <= strike) :
    ...
    def __init__(self, strike, n, r, S, mu, sigma, dt, T, alpha =None, beta=None, rho=None, vol_var=None):
         if alpha is None:
            self.price = self.simulate_price_gbm(strike, n, r, S, mu, sigma, dt, T)
         else:
            inst_var = np.sqrt(sigma)
            self.price = self.simulate_price_svm(strike, n, S, mu, r, dir, alpha, beta, rho, vol_var, inst_var, dt, T)
                     ...
                     if up :
                         if(price >= barrier):
                             ...
                             if(price <= barrier) :
                                 barrier_triggred = True
                                 break
             if out and not  barrier_triggred:
                 if SVM.simulate_path[-1] >= strike:
                     payouts.append((SVM.stimulated_path[-1] - strike)*np.exp(-r*T))
                 else:
                     payouts.append(0)
             elif not out and barrier_triggered:
                 if SVM.simulated_path[-1] >= strike:
...
class MonteCarloAsianCall:
    ...
    def simulate_price_svm(self, strike, n ,S, mu, r , div, alpha, beta, rho, vol_var, inst_var, dt, T ):
        ...
        for i in range(0,n):
            SVM = StochasticVarianceModel(S, mu , r, div, alpha, beta, rho, vol_var, inst_var, dt, T)
            if(np.average(SVM.stimulated_path) >= strike) :
